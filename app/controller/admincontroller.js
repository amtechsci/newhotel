...
exports.edit_hotel = function(req, res) {
    session=req.session;
    if(session.admin_id !== undefined){
        config.con.query("SELECT * FROM hotel WHERE id="+req.params.id,function (err, result) {
            res.render('super_admin/edit_hotel',{APP_URL : config.APP_URL,hotel:result[0]});
        });
    }else{
        res.redirect('/admin/login');
    }
